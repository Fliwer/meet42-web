...
export default function ProfilePage() {
    ...
    return (
        <div className="mx-auto flex max-w-sm flex-col gap-3 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
            <h1 className="mb-2 text-xl font-bold text-indigo-950">Profil</h1>
            <p className="text-sm text-zinc-700">Email : {user.email}</p>
            <p className="text-sm text-zinc-700">Nom : {user.display_name}</p>
            <button onClick={handleLogout} className="mt-2 rounded-md border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50">
                Se déconnecter
            </button>
            <button onClick={handleDelete} className="rounded-md border border-red-200 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50">
